...
import type { Company, HeatmapCell, MapBounds } from '../../types'
import { MAX_VIEWPORT_DIAGONAL_KM, getBoundsDiagonalKm } from '../../hooks/useCompanySearch'
...
function createMarkerIcon(segmento: string, isEnriched: boolean) {
  ...
  const html = renderToString(
    <div style={{
      ...
      <span style={{ transform: 'rotate(45deg)', color: 'white', fontSize: 14, fontWeight: 'bold' }}>
        {segmento[0]}
      </span>
    </div>
  )
...
function UserLocationCenter({ onReady, onDenied }: { onReady: () => void; onDenied: () => void }) {
  const map = useMap()
  ...
  useEffect(() => {
    ...
    if (saved) {
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(
          (pos) => {
            localStorage.setItem(USER_LOCATION_KEY, JSON.stringify({ lat: pos.coords.latitude, lng: pos.coords.longitude }))
          },
          () => {},
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        localStorage.setItem(USER_LOCATION_KEY, JSON.stringify({ lat: loc[0], lng: loc[1] }))
        map.setView(loc, 15)
        onReady()
      },
      () => {
...
function DrawingOverlay() {
  ...
  const zoomToCircle = useCallback((center: [number, number], radius: number) => {
    const circleBounds = L.latLng(center[0], center[1]).toBounds(radius * 2.4)
    map.fitBounds(circleBounds, { padding: [30, 30], maxZoom: 15 })
  }, [map])

  // Zoom map to fit polygon bounds
  const zoomToPolygon = useCallback((points: [number, number][]) => {
  ...
  const isNearFirstPoint = useCallback((latlng: L.LatLng): boolean => {
    const pts = polygonPointsRef.current
    if (pts.length < 3) return false
    const firstPointPx = map.latLngToContainerPoint(L.latLng(pts[0][0], pts[0][1]))
  ...
  useEffect(() => {
    const onMouseMove = (e: L.LeafletMouseEvent) => {
      if (modeRef.current === 'polygon' && polygonPointsRef.current.length > 0) {
  ...
  const mousePreviewLine = useMemo((): [number, number][] | null => {
    if (!mousePos || polygonPoints.length === 0 || mode !== 'polygon' || area) return null
    ...
    <>
      ...
      >
        <button
          ...
          className={`flex items-center gap-2 px-3 py-2 rounded-lg shadow-md text-sm font-medium transition-colors ${
            mode === 'circle' ? 'bg-primary text-white' : 'bg-card text-text hover:bg-surface-alt'
          }`}
        >
          <FiMapPin size={16} />
          Raio
        ...
        <button
          ...
          className={`flex items-center gap-2 px-3 py-2 rounded-lg shadow-md text-sm font-medium transition-colors ${
            mode === 'polygon' ? 'bg-primary text-white' : 'bg-card text-text hover:bg-surface-alt'
          }`}
        >
          <FiStar size={16} />
          Polígono
      ...
      {mode === 'circle' && (
        ...
        >
          ...
          />
          <span className="text-sm font-medium text-text">{(circleRadius / 1000).toFixed(0)} km</span>
          <span className="text-xs text-text-muted ml-2">Clique no mapa para posicionar</span>
        </div>
      )}
      ...
      {mode === 'polygon' && (
        ...
        >
          <span className="text-sm text-text-muted">
            ...
            {polygonPoints.length >= 3 && ' — clique no primeiro ponto para fechar'}
          </span>
        </div>
      )}
      ...
      {mousePreviewLine && (
        <Polyline
          ...
          pathOptions={{ color: '#f97316', weight: 2, opacity: 0.6, dashArray: '4,8' }}
        />
      )}
    </>
  )
      ...
      {showHeatmapPrompt && (
        <div className="absolute inset-0 z-[1500] flex items-center justify-center pointer-events-none">
          <div className="bg-card rounded-xl shadow-2xl px-6 py-5 max-w-sm pointer-events-auto border border-border">
            <h3 className="font-bold text-text text-base mb-2">Mapa de Concentração</h3>
            <p className="text-sm text-text-muted mb-4">
              Deseja visualizar o mapa de concentração para este segmento? O mapa será afastado para mostrar as regiões com maior densidade de empresas.
            </p>
...
function ViewModeLabel() {
  ...
  const label = isHeatmap ? 'Visualização de segmentos' : 'Visualização de empresas'
...
function HeatmapOverlay() {
  ...
  const handleCellClick = useCallback((cell: HeatmapCell) => {
    const bounds = L.latLngBounds(cell.coordinates.map((c) => L.latLng(c[0], c[1])))
        ...
        >
          <Popup>
            <div className="text-center p-1">
              ...
              <p className="text-xs text-text-muted mt-1">Clique para aproximar</p>
            </div>
          </Popup>
        </Polygon>
      ))}
  ...
  useEffect(() => {
    ...
    const watchId = navigator.geolocation.watchPosition(
      (pos) => {
        ...
        const newOrigin: [number, number] = [pos.coords.latitude, pos.coords.longitude]
        localStorage.setItem(USER_LOCATION_KEY, JSON.stringify({ lat: newOrigin[0], lng: newOrigin[1] }))
    ...
    <div className="absolute bottom-6 right-4 z-[1000]">
      <div className="bg-card/95 backdrop-blur-sm rounded-xl shadow-2xl border border-border px-6 py-5 space-y-3 min-w-[240px]">
        ...
        <div className="flex items-center gap-6">
          <div>
            <p className="text-[11px] text-text-muted uppercase tracking-wide mb-0.5">Distância</p>
            <p className="text-xl font-bold text-text">{distKm}</p>
          </div>
          <div className="w-px h-10 bg-border" />
          <div>
            <p className="text-[11px] text-text-muted uppercase tracking-wide mb-0.5">Tempo</p>
...
export default function MapView() {
  ...
  const [locationState, setLocationState] = useState<'ready' | 'loading' | 'denied'>(
    initialLocation ? 'ready' : 'loading'
  )
  const handleLocationReady = useCallback(() => setLocationState('ready'), [])
  const handleLocationDenied = useCallback(() => setLocationState('denied'), [])
  const filteredCompanies = useSearchStore((s) => s.filteredCompanies)
  ...
  const handleEnrich = useCallback((company: Company) => {
        ...
        <Popup maxWidth={320} minWidth={280}>
          <div className="p-1">
            <div className="flex items-start justify-between gap-2 mb-2">
              <h3 className="font-bold text-base text-text leading-tight">{company.nome_fantasia}</h3>
              <span
            ...
            <p className="text-xs text-text-muted mb-1">{company.razao_social}</p>
            <p className="text-xs text-text-muted mb-1">CNPJ: {company.cnpj}</p>
            <p className="text-xs text-text-muted mb-1">CNAE: {company.cnae_descricao}</p>
    ...
    <div className="relative h-full w-full">
      {locationState === 'loading' && (
        <div className="absolute inset-0 z-[2000] bg-surface/80 backdrop-blur-sm flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm text-text-muted font-medium">Obtendo sua localização...</p>
        </div>
      )}
      {locationState === 'denied' && (
        <div className="absolute inset-0 z-[2000] bg-surface/80 backdrop-blur-sm flex flex-col items-center justify-center gap-3">
          <FiMapPin size={32} className="text-primary" />
          <p className="text-sm text-text font-medium">Localização bloqueada pelo navegador</p>
          <p className="text-xs text-text-muted text-center max-w-xs">
            Clique no ícone de cadeado/localização na barra de endereço e permita o acesso à localização.
          </p>
      ...
      >
        ...
        <UserLocationCenter onReady={handleLocationReady} onDenied={handleLocationDenied} />
        <MapBoundsReporter />
        <FlyToHandler />
        <DrawingOverlay />
        <HeatmapOverlay />
